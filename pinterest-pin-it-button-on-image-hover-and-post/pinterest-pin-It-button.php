 ...
 * Text Domain: pinterest-pin-it-button-on-image-hover-and-post
 *
...
define('WEBLIZAR_PINIT_TD', 'pinterest-pin-it-button-on-image-hover-and-post');
...
function front_jquery()
{
	...
	wp_enqueue_script('wl-pin-main', WEBLIZAR_PINIT_PLUGIN_URL . 'js/main.js', array(), '4.9', true);
}
...
function PiniIt_Menu_Assets()
{
	if (current_user_can('manage_options')) {
		wp_register_style('bootstrap', WEBLIZAR_PINIT_PLUGIN_URL . 'css/bootstrap.min.css', array(), '4.9');
		wp_enqueue_style('bootstrap');
		wp_register_style('weblizar-smartech-css', WEBLIZAR_PINIT_PLUGIN_URL . 'css/weblizar-smartech.css', array(), '4.9');
		wp_enqueue_style('weblizar-smartech-css');
		wp_enqueue_script('jquery');
		wp_register_script('bootstrap', WEBLIZAR_PINIT_PLUGIN_URL . 'js/bootstrap.bundle.min.js', array(), '4.9', true);
...
{
	if (isset($_POST['PinItSettingNonce']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['PinItSettingNonce'])), 'pinitsetting_nonce_action')) {
		if (current_user_can('manage_options')) {
			$PinItPost = isset($_POST['PinItPost']) ? sanitize_text_field(wp_unslash($_POST['PinItPost'])) : '';
			$PinItPage = isset($_POST['PinItPage']) ? sanitize_text_field(wp_unslash($_POST['PinItPage'])) : '';
			$PinItOnHover = isset($_POST['PinItOnHover']) ? sanitize_text_field(wp_unslash($_POST['PinItOnHover'])) : '';
			$PinItStatus = isset($_POST['PinItStatus']) ? sanitize_text_field(wp_unslash($_POST['PinItStatus'])) : '';
			$PinItSize = isset($_POST['PinItSize']) ? sanitize_text_field(wp_unslash($_POST['PinItSize'])) : '';
			$PinItDesign = isset($_POST['PinItDesign']) ? sanitize_text_field(wp_unslash($_POST['PinItDesign'])) : '';
			$PinItColor = isset($_POST['PinItColor']) ? sanitize_hex_color(wp_unslash($_POST['PinItColor'])) : '';
...
{
	if (isset($_POST['pinit_exclude_nonce_field']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['pinit_exclude_nonce_field'])), 'pinit_exclude_nonce_action')) {
...
{
	if (isset($_POST['pinit_exclude_page_nonce_field']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['pinit_exclude_page_nonce_field'])), 'pinit_exclude_page_nonce_action')) {
...
function exclude_image_delete()
{
	if (isset($_POST['pinit_exclude_nonce_field']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['pinit_exclude_nonce_field'])), 'pinit_exclude_nonce_action')) {
		if (current_user_can('manage_options')) {
			$all_exclude_images = get_option('exclude_pin_it_images');
			$img_ids = isset($_POST['img_ids']) ? map_deep(wp_unslash($_POST['img_ids']), 'sanitize_text_field') : array();
			if (is_array($img_ids)) {
...
function exclude_image_page()
{
	if (isset($_POST['pinit_exclude_page_nonce_field']) && wp_verify_nonce(sanitize_key(wp_unslash($_POST['pinit_exclude_page_nonce_field'])), 'pinit_exclude_page_nonce_action')) {
		if (current_user_can('manage_options')) {
			$all_exclude_pages = get_option('excluded_pint_it_pages');
			$page_ids = isset($_POST['page_ids']) ? map_deep(wp_unslash($_POST['page_ids']), 'sanitize_text_field') : array();
...
{
	$pinitbutt_add_pro_link = '<a href="https://weblizar.com/plugins/pinterest-feed-pro/" target="_blank">' . esc_html__('Get Premium', 'pinterest-pin-it-button-on-image-hover-and-post') . '</a>';
	array_unshift($links, $pinitbutt_add_pro_link);
	$settings_link_pinitbutt = '<a href="admin.php?page=pinterest-pinit-button-on-hover">' . esc_html__('Settings', 'pinterest-pin-it-button-on-image-hover-and-post') . '</a>';
